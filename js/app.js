...
const App = {
  ...
  buildCalculatorScreen(){
    ...
    const renderTables = ()=>{
      ...
      document.getElementById("calcSedTable").innerHTML = buildDoseRows(SEDATIVES, wt);
      document.getElementById("calcNmbTable").innerHTML = buildDoseRows(NMB.map(n=>({...n,dose:n.dose,unit:"mg/kg"})), wt);
  ...
  buildFailedAirway(){
    ...
    wrap.innerHTML = FAILED_AIRWAY.map((step,i)=>`
      ${i>0?'<div class="flow-arrow">&#8595;</div>':""}
      <div class="flow-step ${step.id==='cric'?'critical':''}" data-fid="${step.id}"><span class="n">${i+1}</span>${step.label}</div>
      <div class="flow-detail" id="detail-${step.id}">${step.detail}</div>
    `).join("");
    wrap.querySelectorAll("[data-fid]").forEach(el=>{
      el.addEventListener("click", ()=>{
        document.getElementById("detail-"+el.dataset.fid).classList.toggle("open");
      });
    });
  },
  ...
  buildVentilator(){
    ...
    grid.querySelectorAll("[data-vent]").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        ...
        detail.hidden = false;
        detail.innerHTML = `<h3>${v.label}</h3><p class="note">${v.settings}</p>`;
      });
    });
  },
  ...
  buildReportFromRSI(s){
    ...
    const confirmLines = CONFIRMATION_CHECKLIST.map((txt,i)=>`  [${s.confirmChecked.has("confirm"+i)?"x":" "}] ${txt}`).join("\n");
    const text =
